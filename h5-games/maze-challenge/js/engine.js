 ...
 *
 * 兼容：目标设备 WebView ≈ Chromium 47 —— 不用 class / const / let / 箭头函数 /
 * 模板字符串 / 解构 / 对象属性简写 / for...of，统一 ES5 写法，
 * 见 ../docs/STANDARD.md §6。（class 与默认参数在 Chrome 47 上会直接解析失败。）
 */
(function (global) {
  var DIRS = {
  ...
  // 渲染里反复用到的常量数组：替代 for...of 的 [-1,1] / 四邻，
  // 提出来避免每次绘制都新建数组（老设备上 GC 压力）
  var SIDES = [-1, 1];
  ...
  function shade(hex, f) {
    ...
    return "rgb(" + Math.round(r + (t - r) * a) + "," +
                    Math.round(g + (t - g) * a) + "," +
                    Math.round(b + (t - b) * a) + ")";
  }
  function LIGHT(hex, f) { return shade(hex, f); }
  function DARK(hex, f) { return shade(hex, -f); }

  function Game(canvas, audio, callbacks) {
    this.canvas = canvas;
  ...
  Game.prototype.load = function (levelIndex, character) {
    var lv = LEVELS[levelIndex];
    this.levelIndex = levelIndex;
    this.level = lv;
    this.cols = lv.grid[0].length;
    this.rows = lv.grid.length;
    this.grid = lv.grid.map(function (r) { return r.split(""); });
    ...
    // 怪兽
    this.monsters = (lv.monsters || []).map(function (m) {
      var path = m.path.map(function (p) { return [p[0], p[1]]; });
      var segLens = [];
      for (var i = 0; i < path.length - 1; i++)
        segLens.push(Math.abs(path[i + 1][0] - path[i][0]) + Math.abs(path[i + 1][1] - path[i][1]));
      return {
        path: path, segLens: segLens, speed: m.speed || 1.5, seg: 0, dir: 1, t: 0,
        rx: path[0][0], ry: path[0][1], cell: [path[0][0], path[0][1]], face: "right"
      };
    });
    this.resize();
  ...
  Game.prototype.resize = function () {
    var maxW = Math.min(window.innerWidth * 0.96, 1100);
    var maxH = window.innerHeight * 0.8;
    var tile = Math.floor(Math.min(maxW / this.cols, maxH / this.rows));
  ...
  Game.prototype.start = function () {
    this.state = "playing"; this.last = performance.now(); this.loop();
  };
  ...
  Game.prototype.resume = function () {
    if (this.state === "paused") { this.state = "playing"; this.last = performance.now(); this.loop(); }
  };
  ...
  Game.prototype.attemptMove = function (dir) {
    if (this.state !== "playing" || this.freezeTimer > 0 || this.moveLock > 0) return;
    this.facing = dir;
    var d = DIRS[dir];
    var tx = this.px + d.x, ty = this.py + d.y;
    var t = this.cellAt(tx, ty);
    // 硬阻挡：墙 / 火墙 / 冰墙 / 越界
    ...
    var ox = this.px, oy = this.py;
    this.px = tx; this.py = ty;
    this.audio.sfx("step");
    // 香蕉 / 弹簧推力（连锁：连续香蕉 / 弹簧落点再触发）
    var dist = this.resolveForced(dir);
    // 落地结算：爱心
    var ft = this.cellAt(this.px, this.py);
    if (ft === "H") {
  ...
  Game.prototype.resolveForced = function (dir) {
    var dist = 0, guard = 0;
    while (guard++ < 64) {
      var c = this.cellAt(this.px, this.py);
      if (c === "B") {
        var d = DIRS[dir];
        var at = this.cellAt(this.px + d.x, this.py + d.y);
        ...
        var steps = 0;
        while (steps < 2) {
          var at2 = this.cellAt(this.px + d2.x, this.py + d2.y);
          if (at2 === "#" || at2 === "F" || at2 === "I" || at2 === null) break;
          if (at2 === "S") { this.damage("spike"); break; }
          this.px += d2.x; this.py += d2.y; dist++; steps++;
        }
  ...
  Game.prototype.damage = function (src) {
  ...
  Game.prototype.checkHazard = function () {
    var hit = false, ice = false;
    for (var i = 0; i < NEIGHBORS.length; i++) {
      var dx = NEIGHBORS[i][0], dy = NEIGHBORS[i][1];
      var c = this.cellAt(this.px + dx, this.py + dy);
  ...
  Game.prototype.checkMonster = function () {
    for (var i = 0; i < this.monsters.length; i++) {
      ...
      if (m.cell[0] === this.px && m.cell[1] === this.py && this.hitInvuln <= 0) {
        ...
        var kx = this.px + (this.px - m.cell[0]);
        var ky = this.py + (this.py - m.cell[1]);
        var kt = this.cellAt(kx, ky);
  ...
  Game.prototype.updateMonster = function (m, dt) {
    var segLen = m.segLens[m.seg] || 1;
  ...
  Game.prototype.update = function (dt) {
    ...
    for (var i = 0; i < this.monsters.length; i++) this.updateMonster(this.monsters[i], dt);
    this.checkMonster();
  ...
  Game.prototype.win = function () {
    ...
    if (this._raf) cancelAnimationFrame(this._raf);
    this.audio.sfx("win");
    var stars = this.hearts >= 5 ? 3 : this.hearts >= 3 ? 2 : 1;
  ...
  // 每格独立相位，避免所有元素同步抖动
  Game.prototype._phase = function (x, y) { return x * 12.9898 + y * 78.233; };

  Game.prototype.drawTile = function (ctx, x, y, c, T) {
    var px = x * T, py = y * T;
    var cx = px + T / 2, cy = py + T / 2;
    var ph = this._phase(x, y);
    var t = this.time;
    var i, s;
    // 地板（棋盘格草绿）
    var floor = (x + y) % 2 === 0 ? "#cdeeb0" : "#bfe6a0";
    ctx.fillStyle = floor; ctx.fillRect(px, py, T, T);
    // 地面微光（偶发 twinkle，点缀不喧宾）
    var tw = Math.sin(t * 2 + ph * 1.3) * 0.5 + 0.5;
    if (tw > 0.86) {
      ctx.fillStyle = "rgba(255,255,255," + (((tw - 0.86) / 0.14) * 0.5) + ")";
      var tx = cx + Math.sin(ph) * T * 0.22, ty = cy + Math.cos(ph) * T * 0.22;
    ...
    if (c === "#") {
      ...
      // 部分墙顶长苔藓，轻轻摇摆
      if (((x * 7 + y * 13) % 5) === 0) {
        var sway = Math.sin(t * 2 + ph) * T * 0.03;
      ...
      var tongues = 3;
      for (i = 0; i < tongues; i++) {
        var fx = cx + (i - 1) * T * 0.22;
        var h = T * 0.52 * (0.7 + 0.3 * Math.sin(t * 9 + ph + i * 1.7));
        var w = T * 0.15;
        ctx.beginPath();
      ...
      for (i = 0; i < 3; i++) {
        var e = (t * 0.6 + ph * 0.1 + i * 0.37) % 1;
        var ex = cx + Math.sin(t * 3 + i) * T * 0.2;
        var ey = cy + T * 0.3 - e * T * 0.7;
      ...
      ctx.restore();
      for (i = 0; i < 2; i++) {
        var a = t * 1.5 + ph + i * Math.PI;
        var rr = T * 0.3;
        var tw2 = Math.sin(t * 4 + ph + i) * 0.5 + 0.5;
      ...
      var lift = 1 + 0.06 * Math.sin(t * 4 + ph);
      ctx.fillStyle = "#9aa0a6";
      for (i = 0; i < 3; i++) {
        var bx = px + T * (0.2 + i * 0.3);
        var top = py + T * 0.25 / lift;
        ctx.beginPath();
      ...
      var s2 = T * 0.3;
      ctx.beginPath();
      ctx.arc(-s2 * 0.5, -s2 * 0.15, s2 * 0.55, 0, Math.PI * 2);
      ctx.arc(s2 * 0.5, -s2 * 0.15, s2 * 0.55, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(-s2 * 0.95, -s2 * 0.05);
      ctx.lineTo(0, s2 * 0.7);
      ctx.lineTo(s2 * 0.95, -s2 * 0.05);
      ctx.closePath();
      ctx.fill();
      ctx.restore();
      var sp = Math.sin(t * 3 + ph + 2.0);
      if (sp > 0.6) this.sparkle(ctx, cx + T * 0.22, cy + bob - T * 0.22, T * 0.1 * (sp - 0.6) / 0.4);
    }
    else if (c === "B") {
      // 香蕉：左右摇摆（像在逗你踩）
      var ang = Math.sin(t * 2.5 + ph) * 0.22;
      ...
      if (this.springAnim > 0) {
        var p = this.springAnim / 0.42, k2 = Math.sin(p * Math.PI);
        sy1 = 1 - 0.4 * k2; sx1 = 1 + 0.28 * k2;
      } else {
        var k3 = Math.sin(t * 3 + ph);
        sy1 = 1 - 0.12 * k3; sx1 = 1 + 0.08 * k3;
      }
      ctx.save(); ctx.translate(cx, cy); ctx.scale(sx1, sy1);
      ctx.strokeStyle = "#5a8dee"; ctx.lineWidth = Math.max(2, T * 0.08);
      ctx.beginPath();
      for (i = 0; i <= 6; i++) {
        var yy = -T * 0.18 + i * (T * 0.36 / 6);
        var xx = (i % 2 ? -1 : 1) * T * 0.12;
      ...
      var fx0 = cx + T * 0.04, fy0 = cy - T * 0.28, fw = T * 0.3, fh = T * 0.2;
      var wv = Math.sin(t * 6 + ph) * T * 0.05;
  ...
  // 四角星闪光（冰墙闪点 / 爱心闪光复用）
  Game.prototype.sparkle = function (ctx, x, y, r) {
    if (r <= 0.2) return;
  ...
  Game.prototype.drawMonster = function (ctx, m, T) {
    var ph = this._phase(m.cell[0], m.cell[1]);
    var t = this.time;
    var cx0 = m.rx * T + T / 2;
    var cy0 = m.ry * T + T / 2;
    var bob = Math.sin(t * 3 + ph) * T * 0.03;
    var cx = cx0, cy = cy0 + bob;
    // 影子
    ctx.fillStyle = "rgba(0,0,0,.12)";
    ctx.beginPath(); ctx.ellipse(cx, cy0 + T * 0.32, T * 0.3, T * 0.12, 0, 0, Math.PI * 2); ctx.fill();
    // 呼吸缩放
    var breathe = 1 + 0.05 * Math.sin(t * 3.5 + ph);
    ctx.save();
    ...
    // 眼睛（朝移动方向）+ 眨眼
    var ex = m.face === "left" ? -1 : m.face === "right" ? 1 : 0;
    var ey = m.face === "up" ? -1 : m.face === "down" ? 1 : 0;
    var blinkPhase = (t + ph) % 2.8;
    var blinking = blinkPhase > 2.55;
    for (var i = 0; i < SIDES.length; i++) {
      var s = SIDES[i];
      var exx = s * T * 0.12 + ex * T * 0.04;
      var eyy = -T * 0.04 + ey * T * 0.04;
      if (blinking) {
    ...
    var legw = T * 0.1, legSwing = Math.sin(t * 6 + ph) * T * 0.05;
    ctx.beginPath(); ctx.ellipse(-T * 0.14 + legSwing, T * 0.34, legw, legw * 0.6, 0, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(T * 0.14 - legSwing, T * 0.34, legw, legw * 0.6, 0, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
    // 近身警示 "!"（玩家 2 格内）
    var d = Math.abs(m.cell[0] - this.px) + Math.abs(m.cell[1] - this.py);
    if (d <= 2) {
      var ay = cy - T * 0.5 + Math.sin(t * 8) * T * 0.04;
  ...
  Game.prototype.drawPlayer = function (ctx, T) {
    var ix = this.fromX + (this.px - this.fromX) * this.animT;
    var iy = this.fromY + (this.py - this.fromY) * this.animT;
    var cx = ix * T + T / 2, cy = iy * T + T / 2;
    var cx0 = cx, cy0 = cy; // 地面锚点（影子固定）
    // 角色自身震动（受击/推力）：确定性阻尼振荡，避免随机抖动造成的"闪"
    if (this.shakeT > 0) {
      var k = this.shakeT / 0.32;                 // 1 -> 0，随时间收敛
      var m = k * T * 0.14;
    ...
    var sx = 1, sy = 1;
    if (this.springAnim > 0) {
      var p = this.springAnim / 0.42;        // 1 -> 0
      var k2 = Math.sin(p * Math.PI);         // 0 -> 1 -> 0
      sy = 1 - 0.4 * k2; sx = 1 + 0.28 * k2;
    ...
    var hurtA = this.hurtFlash > 0 ? Math.min(1, this.hurtFlash / 0.45) : 0;
    if (hurtA > 0) {
      ctx.save();
  ...
  Game.prototype.drawChar = function (ctx, ch, cx, cy, T, facing) {
    var base = ch.color;
    var dark = DARK(ch.color, 0.22);
    var light = LIGHT(ch.color, 0.28);
    var id = ch.id;
    var i, j, s, o;
    // 朝向偏移（眼睛/特征看向移动方向）
    var fx = facing === "left" ? -1 : facing === "right" ? 1 : 0;
    var fy = facing === "up" ? -1 : facing === "down" ? 1 : 0;
    ...
    for (i = 0; i < SIDES.length; i++) {
      s = SIDES[i];
      ctx.beginPath(); ctx.ellipse(cx + s * T * 0.2, cy + T * 0.08, T * 0.06, T * 0.04, 0, 0, Math.PI * 2); ctx.fill();
    }
    // 嘴
    ...
    if (id === "dog") {
      ...
      var WHISKER = [-0.03, 0.03];
      for (i = 0; i < SIDES.length; i++) {
        s = SIDES[i];
        for (j = 0; j < WHISKER.length; j++) {
          o = WHISKER[j];
          ctx.beginPath();
    ...
    for (i = 0; i < SIDES.length; i++) {
      s = SIDES[i];
      ctx.beginPath(); ctx.ellipse(cx + s * T * 0.14, cy + T * 0.34, T * 0.08, T * 0.06, 0, 0, Math.PI * 2); ctx.fill();
    }
  };
