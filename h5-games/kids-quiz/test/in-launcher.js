/*
 * test/in-launcher.js —— 端到端：在「掌中灵 TV 游戏厅」里真的打开 kids-quiz
 *
 * 冒烟测试只能证明游戏自己能跑；这个脚本证明**接入是对的**：
 * 启动器菜单里能翻到它、点进去 iframe 加载的是 kids-quiz/index.html、
 * 并且 iframe 里的 Art / Quiz / TVInput 三个全局都挂上了（脚本没 404）。
 *
...
(async function main() {
  const chrome = spawn(CHROME, [
    ...
    '--user-data-dir=' + path.join(require('os').tmpdir(), 'kq-chrome-launcher'),
    'about:blank'
  ], { stdio: 'ignore' });
  ...
  try {
    ...
    ok(/小小达人/.test(titles), '启动器菜单应出现「小小达人」（实际：' + titles.slice(0, 160) + '）');

    // ② 直接用启动器自己的地址拼装函数取 URL，验证路径对得上真实文件
    const url = await evalJs(
      'window.APP_CONFIG && window.APP_CONFIG.games.filter(function(g){return g.id==="kids-quiz";})[0]' +
      ' ? (function(){return "OK";})() : "MISSING"');
    ok(url === 'OK', 'config.js 里应能取到 kids-quiz 条目');

    const built = await evalJs(
      "(function(){var c=window.APP_CONFIG;var g=null;" +
      "for(var i=0;i<c.games.length;i++){if(c.games[i].id==='kids-quiz')g=c.games[i];}" +
      "if(!g)return '';" +
      "var root=(c.useRemote?c.remoteBase:c.localServer)+'/h5-games';" +
      "return root+'/'+g.folder+'/'+(g.entry||'index.html');})()");
    console.log('    启动器拼出的地址：' + built);
    ...
    const inner = await evalJs(
      ...
      "var d=w.document;" +
      "return {art:!!w.Art,quiz:!!w.Quiz,input:!!w.TVInput," +
      "title:d.title,start:!!d.getElementById('btnStart')," +
      "canvas:!!d.getElementById('game')};}catch(e){return {err:String(e)};}})()");

    ok(!inner.err, '应能访问 iframe 内部（' + (inner.err || 'no error') + '）');
    ok(inner.art && inner.quiz && inner.input,
      'iframe 内 Art/Quiz/TVInput 应全部就绪（' + inner.art + '/' + inner.quiz + '/' + inner.input + '）');
    ok(inner.start && inner.canvas, 'iframe 内应有 #game canvas 与「开始答题」按钮');
    ok(/认知问答/.test(inner.title || ''), 'iframe 标题应为认知问答页（实际「' + inner.title + '」）');
